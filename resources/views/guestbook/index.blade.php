...
<head>
    ...
    <style>
        /* Custom CSS to position the camera box below the Keperluan form */
        .camera-box {
            margin-top: 20px; /* Add spacing from the Keperluan field */
            text-align: center; /* Center the camera box */
        }
    </style>
</head>
...
<script>
    ...
    document.getElementById('guestbookForm').addEventListener('submit', function(e) {
     ...
     const phoneInput = document.querySelector('input[name="nope"]');

phoneInput.addEventListener('input', function() {
    const phone = phoneInput.value;

    // Jika nomor telepon sudah dimasukkan
    if (phone.length >= 12) {  // Tentukan panjang nomor yang valid
        fetch(`/get-guest-by-phone?nope=${phone}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    // Mengisi otomatis form dengan data yang ada
                    document.querySelector('input[name="name"]').value = data.data.name;
                    document.querySelector('input[name="alamat"]').value = data.data.alamat;
                    document.querySelector('input[name="asal_instansi"]').value = data.data.asal_instansi;
                } else {
                    alert(data.message);
                }
            });
